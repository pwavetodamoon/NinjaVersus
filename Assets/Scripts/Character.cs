...
public class Character : MonoBehaviour
{
    ...
    public List<GameObject> ListItemToDrop;

    public float SlideSpeed = 10f;

    public int Coin;
    public bool IsLevelup = false;
    [FormerlySerializedAs("_playerCharacterSO")] public CharacterSO PlayerCharacterSO;
    ...
    private void Start()
    {
        _level = PlayerCharacterSO.GetCurrentLevel();
    }
    ...
    public void DropItem(){
        ...
        // }
        if(ListItemToDrop !=null)
        {
            int randomItem = Random.Range(0, ListItemToDrop.Count);
            Instantiate(ListItemToDrop[randomItem],transform.position, Quaternion.identity);
        }
    }
...
public AudioClip[] HealSounds;
    ...
    {
        PlayerCharacterSO.SetCurrentExp(value);
    }
    
    public void CheckLevelUp()
    {
        if (PlayerCharacterSO.GetCurrentExp() >= PlayerCharacterSO.GetExpToUpNextLevel())
        {
            IsLevelup = true;
            PlayerCharacterSO.SetCurrentExp(-(PlayerCharacterSO.GetCurrentLevel() * 100));
            PlayerCharacterSO.SetLevelUp(1);
            PlayerCharacterSO.SetExpToUpNextLevel((PlayerCharacterSO.GetCurrentLevel() * 100) - (PlayerCharacterSO.GetExpToUpNextLevel()));
        }
    }
